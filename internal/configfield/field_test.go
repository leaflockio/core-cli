...
func TestField_Value_defaultWhenNotOverridden(t *testing.T) {
	f := configfield.Field[string]{Default: "standard"}
...
func TestField_Value_overrideAfterDecode(t *testing.T) {
	...
	c := cfg{Format: configfield.Field[string]{Default: "standard"}}
...
func TestField_key_explicit(t *testing.T) {
	...
	c := cfg{Format: configfield.Field[string]{Key: "fmt", Default: "standard"}}
	if err := configfield.Decode(map[string]any{"fmt": "spdx"}, &c); err != nil {
		t.Fatalf("Decode: %v", err)
	}
	if got := c.Format.Value(); got != "spdx" {
		t.Errorf("Value() = %q, want %q", got, "spdx")
...
func TestField_key_snakeCaseFallback(t *testing.T) {
	type cfg struct {
		UpdateLicenseFile configfield.Field[bool]
		TTL               configfield.Field[int]
	}
	c := cfg{
		UpdateLicenseFile: configfield.Field[bool]{Default: false},
		TTL:               configfield.Field[int]{Default: 24},
	}
...
func TestField_setOverride_scalarSuccess(t *testing.T) {
	...
	c := cfg{Count: configfield.Field[int]{Default: 1}}
	if err := configfield.Decode(map[string]any{"count": 5}, &c); err != nil {
		t.Fatalf("Decode: %v", err)
	}
...
func TestField_setOverride_optionRejected(t *testing.T) {
	...
	err := configfield.Decode(map[string]any{"format": "made-up"}, &c)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestField_setOverride_validateRejected(t *testing.T) {
	...
	err := configfield.Decode(map[string]any{"count": -5}, &c)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestField_setOverride_optionsOnStructField_rejected(t *testing.T) {
	...
	err := configfield.Decode(map[string]any{"nested": map[string]any{"a": true}}, &c)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
}
...
func TestField_setOverride_structSuccess(t *testing.T) {
	type cfg struct {
		Nested configfield.Field[nestedFieldFixture]
	}
	c := cfg{
		Nested: configfield.Field[nestedFieldFixture]{
			Default: nestedFieldFixture{
				A: configfield.Field[bool]{Default: true},
				B: configfield.Field[bool]{Default: true},
			},
		},
	}
	section := map[string]any{
...
func TestField_setOverride_structRawNotAMapping(t *testing.T) {
	...
	c := cfg{Nested: configfield.Field[nestedFieldFixture]{Default: nestedFieldFixture{}}}
	err := configfield.Decode(map[string]any{"nested": "not-a-map"}, &c)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
}
...
func TestField_setOverride_structRecursiveDecodeError(t *testing.T) {
	type cfg struct {
		Nested configfield.Field[badNestedFieldFixture]
	}
	c := cfg{Nested: configfield.Field[badNestedFieldFixture]{Default: badNestedFieldFixture{}}}
	section := map[string]any{
