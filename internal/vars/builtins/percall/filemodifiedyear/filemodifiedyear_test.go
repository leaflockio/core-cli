...
func TestFileModifiedYear_fallsBackToCurrentYearWithNoCommitHistory(t *testing.T) {
	requireGit(t)
	dir := t.TempDir()
	runGit(t, dir, "init")
	...
	got, err := v.Resolve("{FILE_MODIFIED_YEAR}")
	if err != nil {
...
func TestFileModifiedYear_registersAndResolvesFromRealCommit(t *testing.T) {
	requireGit(t)
	dir := t.TempDir()
	runGit(t, dir, "init")
	...
	if FileModifiedYear == nil {
		t.Fatal("FileModifiedYear is nil — registration failed")
	}
	v, err := vars.New(nil)
	if err != nil {
		t.Fatalf("vars.New: %v", err)
