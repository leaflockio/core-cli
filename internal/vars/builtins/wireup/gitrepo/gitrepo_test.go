...
func TestCompute_returnsRepoNameWhenSet(t *testing.T) {
	a := &app.App{Repo: &repo.Info{RepoName: "core-cli"}}
	...
	if got != "core-cli" {
		t.Errorf("compute = %q, want %q", got, "core-cli")
	}
}
...
func TestGitRepo_registersAndResolves(t *testing.T) {
	if GitRepo == nil {
		t.Fatal("GitRepo is nil — registration failed")
	}

	v, err := vars.New(&app.App{Repo: &repo.Info{RepoName: "core-cli"}})
	if err != nil {
		t.Fatalf("vars.New: %v", err)
	}
	got, err := v.Resolve("{GIT_REPO}")
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if got != "core-cli" {
		t.Errorf("Resolve = %q, want %q", got, "core-cli")
	}
}
